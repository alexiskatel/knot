    ...
    <Pressable onPress={onPress} onLongPress={onLongPress} style={[styles.card, isSelected && styles.cardSelected]}>
      ...
      <View style={styles.cardContent}>
        ...
        <View style={styles.cardRow}>
          <Text style={styles.cardTitle} numberOfLines={2}>{tache.titre}</Text>
...
export default function TachesScreen() {
  ...
  const [selectedPriorite, setSelectedPriorite] = useState<Priorite | null>(null);
  ...
  const filtered = (() => {
    ...
    if (selectedPriorite) {
      result = result.filter((t) => t.priorite === selectedPriorite);
    }
    if (searchQuery.trim()) {
  ...
  const hasFilter = selectedProjet !== null || selectedStatut !== 'all' || selectedMembreId !== null || selectedPriorite !== null;
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      {/* Filtres statuts — scroll horizontal, jamais sur 2 lignes */}
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.statutFiltersScroll}>
        ...
        {STATUTS.filter((s) => s.key !== 'done').map((s) => (
          <Pressable
            key={s.key}
      ...
      <View style={styles.membreFilterRow}>
        ...
        {/* Filtre priorité */}
        {(() => {
          const cfg = selectedPriorite ? PRIORITES.find((p) => p.key === selectedPriorite) : null;
          return (
            <>
              <Pressable
                onPress={() => setShowPrioritePicker(true)}
                style={[styles.membreDropdown, cfg && { backgroundColor: cfg.color + '0E', borderColor: cfg.color }]}
              >
                <Ionicons name="flag-outline" size={14} color={cfg ? cfg.color : Colors.textSecondary} />
                <Text style={[styles.membreDropdownText, cfg && { color: cfg.color }]} numberOfLines={1}>
                  {cfg ? cfg.label : 'Priorité'}
                </Text>
                <Ionicons name="chevron-down" size={13} color={cfg ? cfg.color : Colors.textSecondary} />
              </Pressable>
              {selectedPriorite !== null && (
                <Pressable onPress={() => setSelectedPriorite(null)} style={styles.membreClearBtn} hitSlop={8}>
                  <Ionicons name="close-circle" size={16} color={Colors.textDisabled} />
                </Pressable>
              )}
            </>
      ...
      {/* Modal picker priorité */}
      <Modal visible={showPrioritePicker} transparent animationType="slide">
        <Pressable style={styles.modalOverlay} onPress={() => setShowPrioritePicker(false)} />
        <View style={styles.modalSheet}>
          <View style={styles.modalHandle} />
          <Text style={styles.modalTitle}>Filtrer par priorité</Text>
          <Pressable
            onPress={() => { setSelectedPriorite(null); setShowPrioritePicker(false); }}
            style={[styles.modalOption, selectedPriorite === null && styles.modalOptionSelected]}
          >
            <Ionicons name="flag-outline" size={20} color={selectedPriorite === null ? Colors.primary : Colors.textSecondary} />
            <Text style={[styles.modalOptionText, selectedPriorite === null && { color: Colors.primary, fontWeight: '600' }]}>
              Toutes les priorités
            </Text>
            {selectedPriorite === null && <Ionicons name="checkmark" size={18} color={Colors.primary} />}
          </Pressable>
          {PRIORITES.map((p) => (
            <Pressable
              key={p.key}
              onPress={() => { setSelectedPriorite(p.key); setShowPrioritePicker(false); }}
              style={[styles.modalOption, selectedPriorite === p.key && styles.modalOptionSelected]}
            >
              <Ionicons name="flag" size={20} color={p.color} />
              <Text style={[styles.modalOptionText, selectedPriorite === p.key && { color: p.color, fontWeight: '600' }]}>
                {p.label}
              </Text>
              {selectedPriorite === p.key && <Ionicons name="checkmark" size={18} color={p.color} />}
            </Pressable>
          ))}
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  // Statut filters
  statutFiltersScroll: { flexGrow: 0, flexShrink: 0 },
  statutFilters: {
    flexDirection: 'row',
    paddingHorizontal: Layout.screenPaddingH,
    paddingRight: Layout.screenPaddingH + 4,
    gap: 6,
    paddingBottom: 10,
    alignItems: 'center',
  },
  ...
  assigneText: { fontSize: 12, color: Colors.textSecondary, fontWeight: '500', maxWidth: 110 },
